...
class TextEmbeddingGenerator:
    ...
    def generate_embedding(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"Embedding generated in {result['processing_time']:.4f} seconds"
            )
            return result
                ...
                results.append(
                    {
                        ...
                        "model": self.model_name,
                        "processing_time": (time.time() - start_time)
                        / len(texts),  # Approximate per-item time
                    }
                )

            logger.info(
                f"Batch embeddings generated in {time.time() - start_time:.2f} seconds"
            )
            return results
