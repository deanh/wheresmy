...
def print_command_help():
    ...
    print("=========================")
    print(
        "This tool allows you to search for images using content and context information."
    )
    print("\nAvailable commands:")
    print(
        "  search  - Search for images by content, date, camera, and other properties"
    )
...
def main():
    ...
    content_group.add_argument(
        "--query", "-q", help="Text search query (searches descriptions and metadata)"
    )
    ...
    semantic_group.add_argument(
        "--semantic", help="Semantic search query (finds semantically similar content)"
    )
    semantic_group.add_argument(
        "--hybrid", help="Hybrid search combining text and semantic search"
    )
    semantic_group.add_argument(
        "--weight",
        type=float,
    ...
    semantic_group.add_argument(
        "--model", help="Embedding model to use (default: all-MiniLM-L6-v2)"
    )
    ...
    context_group.add_argument(
        "--month", type=int, help="Filter by specific month (1-12)"
    )
    ...
    props_group.add_argument(
        "--gps", help="Filter by GPS location (latitude,longitude,radius_km)"
    )
    ...
    output_group.add_argument(
        "--full-desc", action="store_true", help="Show full descriptions"
    )
    ...
    image_parser = subparsers.add_parser(
        "image", help="Show information about a specific image"
    )
    ...
    if args.command == "search":
        try:
            ...
            if args.gps:
                try:
                    ...
                    logger.warning(
                        "Invalid GPS coordinates format. Use latitude,longitude"
                    )
            ...
            if args.semantic:
                ...
                if weight != args.weight:
                    logger.warning(
                        f"Weight value {args.weight} out of range, using {weight}"
                    )
            ...
            if args.json:
                ...
                for result in results:
                    ...
                    if result.get("gps_lat") and result.get("gps_lon"):
                        print(
                            f"Location: {result.get('gps_lat')}, {result.get('gps_lon')}"
                        )
        ...
        try:
            statistics = stats_utils.get_all_statistics(
                db, date_interval=args.date_interval
            )
            ...
            if args.json:
                ...
                for i, camera in enumerate(statistics["cameras"][:5], 1):
                    print(
                        f"{i}. {camera['make']} {camera['model']}: {camera['count']} images"
                    )
