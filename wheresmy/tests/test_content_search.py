...
def test_date_search(db):
    ...
    if date_stats:
        logger.info("Database contains images from these years:")
        ...
        results = search_utils.search_images(
            db, date_start=start_date, date_end=end_date, limit=10
        )
...
def test_camera_search(db):
    ...
    if camera_stats:
        ...
        if camera_stats:
            ...
            for i, img in enumerate(results[:3], 1):
                logger.info(
                    f"  {i}. {img.get('filename')} - {img.get('camera_make')} {img.get('camera_model')}"  # noqa: E501
                )
...
def test_dimension_search(db):
    ...
    if sample:
        ...
        if widths and heights:
            ...
            logger.info(
                f"Found {len(results)} images wider than or equal to {min_width}px"
            )

            # Display sample results
            for i, img in enumerate(results[:3], 1):
                logger.info(
                    f"  {i}. {img.get('filename')} - {img.get('width')}x{img.get('height')}"
                )

            # Verify we found at least one result
            assert (
                len(results) > 0
            ), f"Expected to find images with width >= {min_width}px"
    else:
...
def test_combined_search(db):
    ...
    if camera_stats and date_stats and len(date_stats) > 0:
        ...
        results = search_utils.search_images(
            db, camera_make=camera, date_start=start_date, date_end=end_date, limit=10
        )
        ...
        for i, img in enumerate(results[:3], 1):
            logger.info(
                f"  {i}. {img.get('filename')} - {img.get('capture_date')} - {img.get('camera_make')}"  # noqa: E501
            )
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test content and context-based search"
    )
