...
def test_search(db_path="image_metadata.db"):
    ...
    if results:
        ...
        if results[0].get('filename'):
            ...
            logger.info(f"Testing text search with term '{search_term}' from filename...")
            text_results = search_utils.search_images(db, text_query=search_term, limit=5)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test search and statistics functionality")
    parser.add_argument("--db", default="image_metadata.db", help="Path to database file")
    parser.add_argument("--test", choices=["search", "stats", "all"], default="all",
