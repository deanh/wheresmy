    ...
    def encode(self, texts, **kwargs):
        ...
        if isinstance(texts, str):
            ...
            for text in texts:
                ...
                embedding = np.ones(384) * (seed / 100.0) + np.arange(384) * (
                    seed / 10000.0
                )
...
class TestTextEmbeddingGenerator(unittest.TestCase):
    ...
    @patch(
        "wheresmy.core.text_embeddings.SentenceTransformer",
    ...
    def test_generate_embeddings_batch(self, mock_sentence_transformer):
        ...
        for i, result in enumerate(results):
            ...
            if i > 0:
                self.assertFalse(
                    np.array_equal(result["embedding"], results[0]["embedding"])
                )
    ...
    @patch(
        "wheresmy.core.text_embeddings.SentenceTransformer", return_value=MagicMock()
    )
