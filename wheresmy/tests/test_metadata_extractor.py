...
class TestMetadataExtractor(unittest.TestCase):
    ...
    def test_extract_metadata_with_vlm(self):
        ...
        self.assertEqual(
            vlm_desc["description"], "This is a mock description of an image."
        )
    ...
    def test_extract_metadata_with_custom_prompt(self):
        ...
        metadata = extract_metadata(
            self.temp_file.name, vlm_describer=self.mock_vlm, vlm_prompt=custom_prompt
        )

        # Check the prompt was passed through
        self.assertEqual(metadata["vlm_description"]["prompt"], custom_prompt)

    def test_process_directory(self):
        """Test processing a directory of images."""
        # Process directory with mock VLM
        results = process_directory(self.temp_dir.name, vlm_describer=self.mock_vlm)
