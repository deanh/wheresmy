...
class IntegrationTest:
    ...
    def extract_metadata(self) -> None:
        ...
        self.run_command(
            [
                "python",
                "-m",
                "wheresmy.core.metadata_extractor",
                "-d",
                self.sample_dir,
                "-o",
                self.metadata_file,
        ...
        self.assert_true(
            images_with_vlm > 0, "At least one image should have a VLM description"
        )
    ...
    def import_metadata(self) -> None:
        ...
        self.assert_true(total_images > 0, "Database should contain images")
        self.assert_true(
            with_descriptions > 0, "Database should contain images with descriptions"
        )
    ...
    def run_search_tests(self) -> None:
        ...
        self.assert_true(len(results) > 0, "Basic search should return results")
        ...
        for keyword in [
            ...
            if results:
                logger.info(
                    f"Found {len(results)} results for content search with keyword '{keyword}'"
                )
                found = True
                break

        self.assert_true(
            found, "Content search should find at least one result with common keywords"
        )
        ...
        if camera_stats:
            ...
            for keyword in [
                ...
                results = search_utils.search_images(
                    db, text_query=keyword, camera_make=camera_make, limit=5
                )
                if results:
                    logger.info(
                        f"Found {len(results)} results for combined search with keyword"
                        "'{keyword}' and camera '{camera_make}'"
                    )
                    found = True
                    break

            self.assert_true(found, "Combined search should find at least one result")
        else:
        ...
        if date_stats:
            ...
            # Search by year
            results = search_utils.search_images(
                db, date_start=f"{year}-01-01", date_end=f"{year}-12-31", limit=5
            )

            self.assert_true(
                len(results) > 0, f"Date search for year {year} should return results"
            )

            if results:
                logger.info(
                    f"Found {len(results)} results for date search with year '{year}'"
                )
        else:
    ...
    def run(self) -> bool:
        ...
        """
        logger.info(
            f"Starting integration test with sample directory: {self.sample_dir}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run integration test for image search"
    )
    parser.add_argument(
        "--keep-files",
        action="store_true",
        help="Don't delete test files after running",
    )
    parser.add_argument(
        "--sample-dir",
